// Bastun MindMate — Cloudflare Pages Function
// Handles POST /api/chat — proxies to Anthropic API
...
function buildContextSection(context) {
  ...
  for (const s of sessions) {
    ...
    const belief = s.coreBelief  || '—';
    const init   = s.initialIntensity ?? 0;
    const final  = s.finalIntensity   ?? 0;
    section += `- ${date}: "${title}" | Core belief: ${belief} | ${init}→${final}/10\n`;
  }
  return section;
}
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    return jsonOut({ error: 'Невалидна заявка.' }, 400);
  }
  ...
  try {
    ...
    return jsonOut({ error: 'Няма връзка с AI услугата.' }, 503);
  }

  if (!apiResponse.ok) {
    const err = await apiResponse.json().catch(() => ({}));
    const msg = err?.error?.message || `API грешка (${apiResponse.status})`;
    return jsonOut({ error: msg }, 502);
  }
  ...
  if (!content) {
    return jsonOut({ error: 'Празен отговор от AI.' }, 500);
  }

  return jsonOut({ response: content });
}
